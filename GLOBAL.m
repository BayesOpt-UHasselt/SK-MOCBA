%% Disclaimer: This code is based on the GLOBAL class of PlatEMO:

%--------------------------------------------------------------------------
% Copyright (c) 2016-2019 BIMK Group. You are free to use the PlatEMO for
...
%Contact author of SKMOCBA <srojas33@gmail.com><sebastian.rojasgonzalez@ugent.be>
...
%   ParameterFile   <public>    obtain the parameter setting from file
    ...
    properties(SetAccess = private)
        algorithm  = @SKMOCBA;       	% Algorithm function
        problem    = @ZDT1;             % Problem function
        ...
        function Validation(obj,value,Type,str,varargin) 
            switch Type
                case 'folder'
                    ...
                    assert(isdir(value),'INPUT ERROR: the folder <%s> does not exist',value); %#ok<*ISDIR>
                case 'function'
    ...
    methods(Access = private,Static)
        ...
        function save(obj)
            ...
            if obj.evaluated >= obj.evaluation
                ...
                Population_nondom   = obj.result{end}(Feasible(NonDominated));
                save(fullfile(folder,sprintf('%s_%s_M%d_%d.mat',func2str(obj.algorithm),func2str(obj.problem),obj.M,obj.run)),'Population','PF','metric','NonDominated','Population_nondom');
            end
        end
        ...
        function show(obj)
            ...
            if obj.evaluated >= obj.evaluation
                ...
                title(sprintf('Deterministic PF'));
        ...
        function cb_metric(hObject,eventdata,obj,metric) %#ok<*INUSL>
